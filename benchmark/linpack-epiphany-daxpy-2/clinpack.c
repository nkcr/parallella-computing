...
>> cc -DSP -DROLL -O4 clinpack.c -lm (nkcr)
*/
...
e_alloc(&shareddy,      BUFOFFSET  + ncores*sizeof(float),  nkn*sizeof(REAL));
e_alloc(&sharedda,      BUFOFFSET  + ncores*sizeof(float) + nkn*sizeof(REAL),  sizeof(REAL));
e_alloc(&shareddx, 			BUFOFFSET  + ncores*sizeof(float) + nkn*sizeof(REAL) + sizeof(REAL),  nkn*sizeof(REAL));
e_alloc(&shared_status, BUFOFFSET  + ncores*sizeof(float) + nkn*sizeof(REAL) + sizeof(REAL) + nkn*sizeof(REAL), ncores*sizeof(int));
...
for(nki=0; nki<rows; nki++) {
  for(nkj=0; nkj<cols; nkj++) {
    e_load("emain.srec", &dev, nki, nkj, E_FALSE);
    e_start(&dev, nki, nkj);
  }
}
...
nkiteration++;
	...
	// -------------------------
      ...
      //printf("looper = %i, status = %i, iteration = %i\n", looper, eCoreStatus, nkiteration);
			// Is eCore Locked ?
			if( eCoreStatus == EFREE ) {
        // set local memory to busy
        // e_write(&shared_status, 0, 0, 0x4*looper, &)
				...
				e_write(&shared_status, 0, 0, 0x4*looper, &ebusy, sizeof(int));
        // e_read(&shared_status, 0, 0, 0x4*looper, &eCoreStatus, sizeof(int));
        // printf("new status = %i\n", eCoreStatus);
